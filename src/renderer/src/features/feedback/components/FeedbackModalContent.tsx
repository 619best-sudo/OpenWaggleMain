  ...
  return (
    <div className="space-y-4 p-4">
      ...
      <label className="block" htmlFor={FEEDBACK_TITLE_INPUT_ID}>
        <span className="mb-1.5 block text-[12px] font-medium text-text-secondary">Title</span>
        <TextInput
      ...
      {fb.error && <p className="text-[12px] text-error">{fb.error}</p>}
    </div>
  )
}
...
function FeedbackAttachmentOptions({
  ...
  return (
    <div className="rounded-md border border-border bg-bg p-3">
      <p className="mb-2 text-[11px] font-medium text-text-tertiary">Include with report</p>
...
function GhCliHelpText({ available }: { readonly available: boolean }) {
  return (
    <span>
      ...
      {available ? (
        <code className="rounded bg-bg px-1 py-0.5 text-[11px]">gh auth login</code>
      ) : (
        <Button
          variant="link"
          size="none"
