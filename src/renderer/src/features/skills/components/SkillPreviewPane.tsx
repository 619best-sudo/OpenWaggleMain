...
function SkillPreviewContent({
  ...
  if (!selectedSkill) {
    return (
      <div className="flex h-full flex-col items-center justify-center text-center opacity-40">
        <p className="text-[12px] font-medium text-text-secondary">No skill selected</p>
  ...
  if (selectedSkill.loadStatus === 'error') {
    return (
      <div className="rounded-xl border border-error/10 bg-error/[0.02] p-5">
        <h4 className="text-[12px] font-semibold text-error/90">Invalid Skill</h4>
        <p className="mt-1.5 text-[11px] leading-relaxed text-error/70">
  ...
  if (isPreviewLoading) {
    return (
      <div className="flex items-center gap-3 py-4 text-[12px] text-text-tertiary/60">
        <Spinner />
...
function SkillPreviewMarkdown({ previewMarkdown }: { readonly previewMarkdown: string }) {
  return (
    <article className="prose max-w-none text-[12px]">
      <ReactMarkdown
