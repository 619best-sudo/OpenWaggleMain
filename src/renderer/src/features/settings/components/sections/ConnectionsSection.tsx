    ...
    <div className="space-y-3">
        ...
        <div className="flex min-w-0 items-start gap-2.5">
          ...
          <div className="min-w-0 space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="rounded-md border border-input-card-border bg-bg-tertiary px-1.5 py-0.5 text-[10px] font-medium text-text-tertiary">
                {count}
              </span>
            </div>
            <p className="max-w-[720px] text-[11px] leading-5 text-text-tertiary">{description}</p>
          </div>
        </div>
      ...
      {isOpen &&
        (count > 0 ? (
          ...
          <p className="px-1 text-[12px] text-text-muted">{emptyText}</p>
        ))}
    </div>
  )
}
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1">
        <h2 className="text-[17px] font-semibold text-text-primary">Connections</h2>
      ...
      {isLoading && providerModels.length === 0 && (
        <p className="text-[12px] text-text-muted">Loading Pi providers…</p>
      )}
