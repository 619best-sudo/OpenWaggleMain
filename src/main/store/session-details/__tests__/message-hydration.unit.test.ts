...
import { hydrateSessionMessages } from '../message-hydration'
...
describe('hydrateSessionMessages', () => {
  it('remaps approved resumed tool results onto the original permission-request tool call', () => {
    ...
    expect(messages.map((message) => String(message.id))).toEqual([
      'user-1',
    ...
    expect(messages[3]?.parts).toMatchObject([
      {
        type: 'tool-result',
        toolResult: {
          id: 'tool-request',
