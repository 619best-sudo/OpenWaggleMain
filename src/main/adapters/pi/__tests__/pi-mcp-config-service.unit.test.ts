...
describe('Pi MCP config service', () => {
  ...
  it('enables and disables the adapter package source without deleting MCP config', () =>
    withFixture(async ({ home, agentDir, project }) => {
      ...
      const service = createPiMcpConfigServiceForTests({
        ...
        warmMcpMetadataCache: async (configPath) => {
          warmedConfigs.push(configPath)
        },
      })
  ...
  it('warms MCP metadata after writing source config when the adapter is enabled', () =>
    withFixture(async ({ home, agentDir, project }) => {
      const warmedConfigs: string[] = []
      const service = createPiMcpConfigServiceForTests({
        homeDir: home,
  ...
  it('auto-enables the adapter at runtime when active MCP servers are configured', () =>
    withFixture(async ({ home, agentDir, project }) => {
      const installs: string[] = []
      const service = createPiMcpConfigServiceForTests({
        homeDir: home,
  ...
  it('preserves explicit direct-tools settings from MCP config sources', () =>
    withFixture(async ({ home, agentDir, project }) => {
      ...
      expect(await readMcpConfig(generatedPath)).toEqual({
        settings: {
          directTools: false,
        },
        mcpServers: {
  ...
  it('toggles only the selected source entry by moving it between active and disabled server maps', () =>
    withFixture(async ({ home, agentDir, project }) => {
      ...
      expect(warmedConfigs).toHaveLength(2)
      expect(await readMcpConfig(projectConfigPath)).toEqual({
        mcpServers: {
          beta: { command: 'beta' },
          alpha: { command: 'alpha' },
  ...
  it('does not fail MCP settings updates when metadata warming fails', () =>
    withFixture(async ({ home, agentDir, project }) => {
      const service = createPiMcpConfigServiceForTests({
        homeDir: home,
        agentDir,
        warmMcpMetadataCache: async () => {
