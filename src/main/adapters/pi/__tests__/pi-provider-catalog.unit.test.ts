...
describe('createPiRuntimeServices', () => {
  it('keeps direct OpenRouter models and adds a single separate turing-machine option', async () => {
    ...
    try {
      ...
      const openRouterModel = services.modelRegistry.getAll().find((model) => model.provider === 'openrouter')
      const turingMachineModel = services.modelRegistry.find('turing-machine', 'greatx-backend')
      ...
      expect(turingMachineModel?.name).toBe('GreatX Backend')
  ...
  it('reloads settings after preparing MCP runtime context so adapter packages are visible', async () => {
    const root = await createTempProject()
    const agentDir = path.join(root, 'pi-agent')
    const home = path.join(root, 'home')
    ...
    try {
      ...
      expect(services.settingsManager.getGlobalSettings().packages).toContain(
        MCP_ADAPTER_PACKAGE_SOURCE,
      )
    } finally {
      await fs.rm(root, { recursive: true, force: true })
