...
function withRuntimeDirectToolsDefault(settings: McpAdapterSettings): McpAdapterSettings {
  if (settings.directTools !== undefined) {
    return settings
  }

  // OpenWaggle does not currently surface pi-mcp-adapter's interactive /mcp UX directly,
  // so expose MCP servers as direct tools by default in the generated runtime config.
  // Also prefer those direct tools over the proxy mcp() tool to avoid fragile JSON-string args.
  return {
    ...settings,
    directTools: true,
    ...(settings.disableProxyTool === undefined ? { disableProxyTool: true } : {}),
  }
}
...
export function buildRuntimeConfigObject(effective: McpEffectiveConfig) {
  const runtimeSettings = withRuntimeDirectToolsDefault(effective.settings)
  return {
    ...(effective.imports.length > 0 ? { imports: [...effective.imports] } : {}),
    ...(Object.keys(runtimeSettings).length > 0 ? { settings: runtimeSettings } : {}),
