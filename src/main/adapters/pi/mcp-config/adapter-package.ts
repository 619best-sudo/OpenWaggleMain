...
async function installedMcpAdapterUsesBundledPackage(installedPath: string, bundledPath: string) {
  try {
    const stats = await lstat(installedPath)
    if (!stats.isSymbolicLink()) {
      return false
    }
    return (await realpath(installedPath)) === (await realpath(bundledPath))
  } catch {
    return false
  }
}
...
async function installBundledMcpAdapterPackage(source: string, agentDir: string) {
  ...
  if (
    (await installedMcpAdapterMatchesSource(source, installedPath)) &&
    (await installedMcpAdapterUsesBundledPackage(installedPath, bundledPath))
  ) {
    return
  }
  ...
  await symlink(bundledPath, installedPath, process.platform === 'win32' ? 'junction' : 'dir')
}
