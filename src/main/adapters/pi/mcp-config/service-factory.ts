...
import { logger, MCP_ADAPTER_PACKAGE_SOURCE } from './constants'
import {
...
async function warmMcpMetadataCacheIfReady(
  ...
  if (!options.warmMcpMetadataCache) {
    return
  }

  if (!view.adapter.enabled || Object.keys(view.effective.mcpServers).length === 0) {
    return
  }
  ...
  try {
    ...
    await options.warmMcpMetadataCache(configPath)
  } catch (error) {
    logger.warn('Failed to warm MCP metadata cache after settings update', {
...
async function prepareRuntimeContext(
  ...
  const hasActiveServers = Object.keys(view.effective.mcpServers).length > 0
  if (!view.adapter.enabled && !hasActiveServers) {
    return null
  }
