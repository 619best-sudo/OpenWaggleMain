...
function plannerPrompt(goal: string) {
  return [
    ...
    '- Require clean, simple, well-structured output (for SVG: minimal, semantic markup with reusable <defs>/gradients; sensible viewBox; no clutter) so each element is simple yet visually rich.',
    ...
    'Example of the expected granularity (illustrative only — adapt to the real request, do not copy verbatim):',
    '- For "a realistic animated solar system": (1) scaffold index.html with a dark space background and a centered stage container; (2) add a subtle starfield layer; (3) add the Sun as a richly shaded SVG using a radial gradient and outer glow; (4) add Mercury as its own SVG with realistic size/color/shading; (5) add Venus as its own SVG; (6..) one task per remaining planet; (N) add the Moon as its own SVG orbiting Earth; then one task each to add slow self-rotation to a specific body; then one task each to add orbital revolution to a specific planet; and a final polish task tuning sizes, orbit timing, and visual consistency.',
    '- Each of those bullets is a separate task with its own rich, standalone prompt.',
