...
- Turing-harness thread continuity is a bounded STEP LEDGER (last 8 runs), projected from the per-run `openwaggle.turing-thread-snapshot` nodes that already accumulate in the persisted tree, merged with the warm session's live slot, injected as `followUpContext.recentRuns` into the first hop's opening only. `ThreadRunSnapshot.task` is the full WRAPPED runtime prompt — the host must stamp the raw user text as `userQuery` when persisting, or the ledger echoes the envelope. Cross-run gate: no continuity when the newest snapshot recommends `fresh` (pending question). The composer context meter under this kernel reads `buildTuringContextUsageSnapshot`, which measures the NEXT request, not a transcript: the newest snapshot's `task` (the full wrapped runtime envelope) plus `renderThreadContinuity(...)` over the same step ledger the next run would inject, at ~4 chars/token, against the resolved model window from `resolveModel(resolveTuringModelSlug(modelRef))` (the slug resolver must never touch credentials — meter polls are frequent). It deliberately excludes the categorizer's own system prompt, which is routing-dependent and not something the thread grows. Real threads read a few hundred to a few thousand tokens — a fraction of a percent of a 262k window — because bounded context is the harness's design, so the meter renders tokens AND percent with one decimal below 1%. `label: 'Next request'` names the numerator in the tooltip; the renderer hook keeps the last snapshot during re-fetches so the reading never bounces to the provider-catalog fallback. Edits to `../turing-harness/src` require `npm run build` there (no `prepare:turing-harness` script exists anymore) and an electron-vite restart, since the main bundle inlines its `dist/`.
